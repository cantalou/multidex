...
public class DexMethodCounts
{
    ...
    public static void generate(Node packageTree, DexData dexData, boolean includeClasses, String packageFilter, int maxDepth, Filter filter)
    {
        ...
        for (MethodRef methodRef : methodRefs)
        {
            ...
            String packageName = includeClasses ? Output.descriptorToDot(classDescriptor)
                                                        .replace('$', '.') : Output.packageNameOnly(classDescriptor);
    ...
    private static MethodRef[] getMethodRefs(DexData dexData, Filter filter)
    {
        ...
        for (MethodRef methodRef : methodRefs)
        {
            ...
            if ((filter == Filter.DEFINED_ONLY && !isExternal) || (filter == Filter.REFERENCED_ONLY && isExternal))
            {
                filteredMethodRefs.add(methodRef);
            }
        }
        out.println("Filtered to " + filteredMethodRefs.size() + " " + (filter == Filter.DEFINED_ONLY ? "defined" : "referenced") + " method IDs.");
